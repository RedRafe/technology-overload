...
local function hasFormula(technology)
    if technology.unit.count_formula ~= nil then
        return true
    end
    return false
end
...
-- @ technology: Prototype/Technology
-- @ visiting: table<string, true>, tracks the current recursion path to detect prerequisite cycles
local function getTechnologyDepth(technology, visiting)
    ...
    -- guard against prerequisite cycles (e.g. cross-mod prerequisites, load order changes)
    visiting = visiting or {}
    if visiting[technology.name] then
        log('[technology-overload] prerequisite cycle detected at "' .. technology.name .. '" - approximating depth as 1 to avoid hanging')
        return 1
    end
    ...
    if hasPrerequisites(technology) and (#technology.prerequisites > 0) then
        local depths = {}
        for _, prerequisite in pairs(technology.prerequisites) do
            table.insert(depths, getTechnologyDepth(data.raw.technology[prerequisite], visiting))
        end
        table.sort(depths)
...
local function BaseDepth(currentDepth, p)
    if not p.applyDepth then
        return Exp(1, p.depthExp)
    end
...
local function Fibonacci(technology, p, visiting)
    -- cached value
    ...
    visiting = visiting or {}
    if visiting[technology.name] then
        log('[technology-overload] prerequisite cycle detected at "' .. technology.name .. '" - approximating cost as current cost to avoid hanging')
        return getTechnologyUnitCount(technology) or 0
    end
    visiting[technology.name] = true
    -- compute value
    ...
    if hasPrerequisites(technology) then
        ...
        for _, tech in pairs(depth) do
            sumFibonacci = sumFibonacci + Fibonacci(tech, p, visiting)
        end
...
-- maxDepth: int,           max depth across the tech tree
-- inverseDepth: bool 0/1,  wheter to apply reverse depth computation
local function exponentialCumulativeCost(technology, p, visiting)
    -- cached value
    ...
    if hasPrerequisites(technology) and p.searchDepth then
        local depths = {}
        for _, prerequisite in pairs(technology.prerequisites) do
            table.insert(depths, exponentialCumulativeCost(data.raw.technology[prerequisite], p, visiting))
        end
